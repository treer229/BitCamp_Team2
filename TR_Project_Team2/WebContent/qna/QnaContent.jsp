...
<%@page import="kr.or.bit.dto.Qna_Comments"%>
...
<body>
	<%
		//out.print(request.getParameter("qna_num") + "<br>");
		//out.print(request.getParameter("cp") + "<br>");
		//out.print(request.getParameter("ps") + "<br>");
		/* String qna_num =	request.getParameter("qna_num"); //글번호
	...
	//조회수 증가
	boolean res = boardcontent.addReadNum(qna_num);
	if(res) System.out.println("조회수 증가");
		
	//데이터 조회 출력(글번호가 없는 게시글에 조회시 ...)
	board boarddto = boardcontent.content(Integer.parseInt(qna_num));//content(Integer.parseInt(qna_num));
	if(boarddto == null){
		response.sendRedirect("board_list.jsp");
		return;
	} */
	...
	QnaDao dao = (QnaDao)request.getAttribute("dao");
			...
			<center>
				...
				<table width="80%" border="1">
					<tr>
						...
						<td width="30%"><%= qna_num %></td>
						<td width="20%" align="center"><b>작성일</b></td>
						<td><%= qna.getCreated_date() %></td>
					</tr>
					<tr>
						<td width="20%" align="center"><b>글쓴이</b></td>
						<td width="30%"><%=qna.getId() %></td>
					...
					<tr>
						...
						<td colspan="3"><%= qna.getQna_title() %></td>
					</tr>
					<tr height="100">
					...
					<tr>
						<td colspan="4" align="center">
				            ...
				            |<a href="boardEditForm.bbs?qna_num=<%= qna_num %>&cp=<%= cpage %>&ps=<%= pagesize %>">편집</a>
				            |<a href="boardDeletePassword.bbs?qna_num=<%= qna_num %>&cp=<%= cpage %>&ps=<%= pagesize %>">삭제</a>
				            |<a href="boardRewrite.bbs?qna_num=<%= qna_num %>&cp=<%= cpage %>&ps=<%= pagesize %>&subject=<%= qna.getQna_title() %>">답글</a>									
						</td>
					</tr>
				</table>
			<!--  꼬리글 달기 테이블 -->
